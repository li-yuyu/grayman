package com.lyle.grayman.agent.classloader;
 ...
 * 跨类加载器缓存
 * <p>
 * agent-plugin包的类虽然都由AgentClassLoader加载，但因为各 * 所加载的上下文，以此达到目的。又因为Thread.currentThread().getContextClassLoader()默认获得的是AppClassLoader，所以将其放在arch-agent包中。个Interceptor拦截时机不一致，即所拦截的类不一致，其类加载器不一致。导致
